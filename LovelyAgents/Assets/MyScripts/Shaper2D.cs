...
public class Shaper2D : MonoBehaviour {
    ...
    //Apply changes after polygons generation
    ...
    void ChangeSectorCount()
    {
        if (sectorCount < 6) sectorCount = 6;
        if (sectorCount % 2 != 0) sectorCount++;
    }

    //Make inner radius inferior to outter radius
    void MakeInnerInfOutter()
    {
        innerRadius = outterRadius - 0.1f;
    }

    //Don't allow inner radius to be bigger than outter radius
    void MakeOutterSupInner()
    {
        outterRadius = innerRadius + 0.1f;
    }

    //Check if changes were made after polygons generation
    bool somethingChanged
    {
		get
        {
            return (CheckMeshColor() || CheckSectorCount() || CheckArcDegrees() || CheckRotation()
                || CheckOutterRadius() || CheckInnerRadius() || CheckStarrines());
        }
    }

    //Check if starrines changed
    bool CheckStarrines()
    {
    ...
    //Check if inner radius changed
    bool CheckInnerRadius()
    {
        if (innerRadius != innerRadiusPrev)
        {
            UpdateInnerRadiusPrev();
            return true;
        }
    ...
    //Check if outter radius changed
    bool CheckOutterRadius()
    {
        if (outterRadius != outterRadiusPrev)
        {
            UpdateOutterRadiusPrev();
            return true;
        }
    ...
    //Check if arc degrees changed
    bool CheckArcDegrees()
    {
        if (arcDegrees != arcDegreesPrev)
        {
            UpdateArcDegreesPrev();
            return true;
        }
        else
        {
    ...
    //Check if sector count changed
    bool CheckSectorCount()
    {
        if (sectorCount != sectorCountPrev)
        {
            UpdateSectorCountPrev();
            return true;
        }
    ...
    private void UpdateStarrinesPrev()
    {
        starrinesPrev = starrines;
    }

    void UpdateInnerRadiusPrev()
    {
        innerRadiusPrev = innerRadius;
    }

    void UpdateOutterRadiusPrev()
    {
        outterRadiusPrev = outterRadius;
    }

    void UpdateRotationPrev()
    {
        rotationPrev = rotation;
    }

    void UpdateArcDegreesPrev()
    {
        arcDegreesPrev = arcDegrees;
    }

    void UpdateSectorCountPrev()
    {
        sectorCountPrev = sectorCount;
    }

    void UpdateMeshColorPrev()
    {
    ...
    //I think it is the original method from Shaper2D class, it should be refactored to make it more clear
    void GenerateCircle(){
		vertices.Clear();
		radiusPlus=0f;
